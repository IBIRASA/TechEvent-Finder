...
export const register = async (req, res) => {
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, 10);
    const result = await query(
      `INSERT INTO users(username, email, password_hash, latitude, l) 
      VALUES($1, $2, $3, $4, ST_SetSRID(ST_MakePoint($5, $6), 4326)) 
      RETURNING user_id`,
      [username, email, hashedPassword, longitude, latitude]
    );
    ...
    res.status(500).json({
      ...
      details: error.details || undefined, // Include any additional details
    });
  }
};
