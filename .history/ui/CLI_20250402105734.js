...
//           askQuestion(index + 1);  // Use index + 1 to move to the next question
//         });
//       };
...
class EventLocatorCLI {
  ...
  registerUser() {
    ...
    this.rl.question(chalk.cyan("Username: "), (username) => {
      this.rl.question(chalk.cyan("Email: "), (email) => {
        this.rl.question(chalk.cyan("Password: "), {hideEchoBack: true}, (password) => {
          this.rl.question(chalk.cyan("Latitude: "), (latitude) => {
            this.rl.question(chalk.cyan("Longitude: "), (longitude) => {
  ...
  async completeRegistration(data) {
    try {
      ...
      console.log(chalk.red(JSON.stringify(error.response?.data, null, 2))); // Detailed error
      this.showAuthMenu();
    }
  }
  ...
  loginUser() {
    ...
    this.rl.question(chalk.cyan("Email: "), (email) => {
      ...
      const onData = (key) => {
        ...
        // Enter key pressed - finish password entry
        if (key === "\r" || key === "\n") {
