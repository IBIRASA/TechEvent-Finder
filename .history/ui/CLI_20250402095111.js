...
class EventLocatorCLI {
  ...
  async completeEventCreation(data) {
    try {
      const response = await this.api.post('/events', {
        ...
        categories: data.categories.split(',').map(id => parseInt(id.trim())),
      ...
      console.log(chalk.white(`Event ID: ${chalk.bold(response.data.eventId)}`));
      this.showMainMenu();
    } catch (error) {
  ...
  async viewMyEvents() {
    try {
      ...
      response.data.forEach(event => {
        const status = new Date(event.start_time) > new Date() 
